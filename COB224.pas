...
procedure TFDevCuoEfec.FormKeyPress(Sender: TObject; var Key: Char);
begin
   If Key = #13 Then
    begin
     Key := #0;
...
procedure TFDevCuoEfec.dbeAsoNomExit(Sender: TObject);
var
 xSQL : String;
begin
...
procedure TFDevCuoEfec.dbeCodigoModularExit(Sender: TObject);
var
 xSql : String;
begin
...
procedure TFDevCuoEfec.dblcBancoExit(Sender: TObject);
var
 xSql,xWhere : String;
begin
...
procedure TFDevCuoEfec.cargabusqueda;
var
 xSql : String;
begin
...
procedure TFDevCuoEfec.LevantaFiltro(xCampo: String);
var
   Mtx : TMant;
begin
   DM1.cdsAsoX.Close;
...
procedure TFDevCuoEfec.OnEditAso(Sender: TObject; MantFields: TFields);
var
  xSQL : String;
begin
   xSQL := 'Select * From APO201'
...
procedure TFDevCuoEfec.cargadocumentos;
var
 xsql,sWhere : String;
begin
  ...
  sWhere:= ' FLGCANC='+quotedstr('S')+' AND TIPO ='+quotedstr('E');
  ...
  xSql := 'select distinct credocpag, credid, TO_DATE(foperac,''DD/MM/YYY'') FECHA'+
          ...
          ' AND CREFPAG='+QuotedStr(DatetoStr(xFechasys))+
          ' AND NVL(CREFLAGDEV,''N'')=''N''';
  dm1.cdsQry1.close;
...
procedure TFDevCuoEfec.dbgDocumenDblClick(Sender: TObject);
var
 xSql : String;
begin
  xSQL:='SELECT '' '' SUMA,ASOID,CREANO,CREMES,CREMTOCOB,CREAMORT,CREINTERES,CREFLAT,NROOPE,FORPAGABR,TIPCREID, '+
        ' CREDID,CRECUOTA,CREMTOCOB,FORPAGOID,FORPAGOABR, CREESTANT, CREESTANTDES '+
        ' FROM CRE310'+
...
procedure TFDevCuoEfec.FormActivate(Sender: TObject);
var
 xWhere,xSQL : string;
begin
...
procedure TFDevCuoEfec.dblcFormaPagoExit(Sender: TObject);
var
 sWhere : String;
begin
...
procedure TFDevCuoEfec.dblctmonedaExit(Sender: TObject);
var
 sWhere : String;
begin
...
procedure TFDevCuoEfec.dblcChqBancoExit(Sender: TObject);
var
 sWhere : String;
begin
...
procedure TFDevCuoEfec.dbeFOperacionExit(Sender: TObject);
begin
  DecodeDate(dbeFOperacion.Date,xAnos,xMesS,xDiaS);
end;

procedure TFDevCuoEfec.dbeChqNumExit(Sender: TObject);
begin
...
procedure TFDevCuoEfec.dbgCuatasAsoFieldChanged(Sender: TObject;
  Field: TField);
var
 xZ,xI : Integer;
 xBookMark : TBookMark;
...
procedure TFDevCuoEfec.Z2bbtnBorrarClick(Sender: TObject);
var
 xI : Integer;
begin
 try
...
procedure TFDevCuoEfec.bbtnSigueClick(Sender: TObject);
var
 ...
 try
  if MessageDlg('Esta Seguro(a) de Devolver',mtconfirmation,[mbYes,MbNo],0)=mrYes then
   begin
    ...
    DM1.cdsDevolucion.fieldbyname('MODO').AsString       := 'DEVOLUCION';
...
{    xTipocorre := '8';
    ...
    while not dm1.cdsAso.eof do
     begin
       if dm1.cdsAso.FieldByName('SUMA').AsString='S' then
       begin
          ...
          FloatToStr(DM1.cdsAso.FieldByName('CREINTERES').AsFloat)+','+
          FloatToStr(DM1.cdsAso.FieldByName('CREFLAT').AsFloat)+','+
          FloatToStr(DM1.cdsAso.FieldByName('CREAMORT').AsFloat)+','+
          FloatToStr(DM1.cdsAso.FieldByName('CREMTOCOB').AsFloat)+','+
          ...
          quotedstr(DM1.cdsPadre.FieldByName('CIUDID').AsString)+','+quotedstr('COB')+','+quotedstr('DEV')+','+
          ...
          if DM1.cdsAso.FieldByName('CREESTANT').AsString<>'' then
          begin
          ...
          DM1.DCOM1.AppServer.EjecutaSQL(xSQL);
             ...
             //** actualizo el cre302
          xSql := 'UPDATE CRE302 SET  CREMTOCOB= nvl(CREMTOCOB,0)-'+''+fLOATTOSTR(DM1.cdsAso.FieldByName('CREMTOCOB').AsFloat)+'';
          xSql := xSql + ' ,AREAID='+QuotedStr('COB');
          xSql := xSql + ' ,CRESALDO = CRESALDO + '+ DM1.cdsAso.FieldByName('CREMTOCOB').AsString;
          xSql := xSql + ' ,CREESTID='+QuotedStr(xEstidZ)+' ,CREESTADO='+QuotedStr(xEstadoZ);
          xSql := xSql + ' ,CRECAPITAL = CRECAPITAL -'+''+FloatToStr(DM1.cdsAso.FieldByName('CREAMORT').AsFloat)+''+
                         ' ,CREMTOINT  = CREMTOINT  -'+''+FloatToStr(DM1.cdsAso.FieldByName('CREINTERES').AsFloat)+''+
                         ' ,CREMTOFLAT = CREMTOFLAT -'+''+FloatToStr(DM1.cdsAso.FieldByName('CREFLAT').AsFloat)+''+
...
//                         ' AND CREANO='+QuotedStr(DM1.cdsAso.FieldByName('CREANO').AsString)+
//                         ' AND CREMES='+QuotedStr(DM1.cdsAso.FieldByName('CREMES').AsString)+
...
/////////////
      sSQL:= ' SELECT SUM(CREMTOCOB) TOT FROM CRE302 '+
             ' Where ASOID ='+QuotedStr(xAsoid)+
             ' AND CREDID='+QuotedStr(xCredId)+
             ' AND CREESTID in ('+QuotedStr(xEstid0)+','+quotedstr(xEstid1)+','+quotedstr(xEstid2)+')';
      DM1.cdsQry.Close;
      DM1.cdsQry.DataRequest(sSQL);
      DM1.cdsQry.Open;
     ...
     ShowMessage('Devolución Aceptada');
  end
  else
    bbtnSigue.Enabled := False;
 finally
...
procedure TFDevCuoEfec.addMeses;
var
 xI : Integer;
...
procedure TFDevCuoEfec.Correlativo;
begin
...
procedure TFDevCuoEfec.CorreCred(pCorre: String);
var
  xCadena,xSql : String;
begin
...
procedure TFDevCuoEfec.BuscaCorre;
var
  xSQL : String;
begin
  //** por defecto
...
procedure TFDevCuoEfec.FormClose(Sender: TObject;
