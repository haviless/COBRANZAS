// Actualizaciones
// HPC_201215_COB : Modificaciones por fondo de desgravamen
...
type
  ...
  public
    ...
    procedure InsertaDetPagos(xCuotaApl, xExceso, xAmoPag, xIntPag, xGasPag :Currency; xEstado:Integer; xEstAnt, xDesAnt, xFrmPag, xCnd:String);
  end;
...
begin
   ...
   If DM1.cdsFormaPago.Locate('FORPAGOID',VarArrayof([DBLCTipCob.text]),[]) Then
      Begin
        ...
        If Trim(DBLCTipCob.text)='31' Then DtpFecPag.OnExit(DtpFecPag);
        ...
        If Trim(DBLCTipCob.text)='31' Then
           Begin
           // Fecha
             DtpFecPag.Text:=FechaAnterior(xAsoid);
             EdtForPago.Text  := DM1.cdsFormaPago.fieldbyname('FORPAGODES').AsString ;
             EdtMonExc.Text := FormatFloat('###,###.#0',CalcExceso(DM1.cdsCuotas.FieldByName('ASOID').AsString, DM1.cdsCuotas.FieldByName('CREDID').AsString));
             If DM1.Valores(EdtMonExc.Text) >= DM1.Valores(EdtMonPag.Text) Then
                EdtMonCob.Text:= EdtMonPag.Text
...
begin
// inicio HPP_201108_COB
// No permite realizar registro si tiene saldo en el FSC
   xSql := 'SELECT COUNT(*) CANT '
         + '  FROM COB_FSC_DEUDA_FSC_CAB B '
         + ' WHERE ASOID = '+QuotedStr(DM1.cdsCuotas.FieldByName('ASOID').AsString)
         + '   AND IMP_SAL_FSC > 0 '
         + '   AND ID_EST_FSC NOT IN (''04'', ''13'', ''99'')';
   ...
   If DM1.cdsQry.FieldByName('CANT').AsFloat > 0 Then
   BEGIN
     TRY
        GrpFormaPago.Enabled := false;
        GrpGrabar.Enabled := false;
        BitGrabar.Enabled := false;
        showmessage('No puede realizar pagos por este medio, porque el asociado Tiene aplicación de FSC'#13
                   +'   Favor indicar al asociado que debe realizar el pago en las agencias bancarias  ');
        BitSalirClick(Sender);
     FINALLY
     END;
     Exit;
   END;
...
Else
   Begin
     ...
     Else
        Begin
           DtpFecPag.Enabled:=True;
           If DBLCTipCob.Enabled Then DBLCTipCob.SetFocus;
        End;
   End;
end;
...
var xAsoId, xCreDid, xFecOto, xCreCuota, xCnd, xFrmPag, xFecCalculo,xRecEfe,xRecExe :String;
...
xFecOto:=Copy(DM1.cdsSolicitud.FieldByName('CREFOTORG').AsString,7,4)+Copy(DM1.cdsSolicitud.FieldByName('CREFOTORG').AsString,4,2)+Copy(DM1.cdsSolicitud.FieldByName('CREFOTORG').AsString,1,2);
If xFrmPag='06' Then
   Begin
      If DM1.Valores(Trim(EdtMonExc.Text))=0 Then
         Begin
            ...
            Exit;
         End;
      If xFecOto<='19961231' Then
         Begin
           MessageDlg('LA FECHA DE OTORGAMIENTO DEL CREDITO ES MENOR AL 31/12/1996  '+#13+' No Procede Aplicación De Excesos Normal De Excesos !!! ', mtError, [mbOk], 0);
           Exit;
         End;
...
If xFrmPag='31' Then
   Begin
      If DM1.Valores(Trim(EdtMonExc.Text))=0 Then
         Begin
            MessageDlg('No Existen Excesos Extemporaneos Por Aplicar,  Verifique !!! ', mtError,[mbOk],0);
            Exit;
         End;

      If xFecOto>'19961231' Then
         Begin
           MessageDlg('LA FECHA DE OTORGAMIENTO DEL CREDITO ES POSTERIOR AL 31/12/1996  '+#13+' No Procede Aplicación Extemporanea !!! ', mtError, [mbOk], 0);
           Exit;
         End;
...
If xFrmPag='31' Then
   Begin
     xRecExe:=GnrCExe_Ex; EdtDocPag.Text:=xRecExe;
     If Length(Trim(xRecExe))=0 Then
         Begin
           Screen.Cursor:=crDefault;
           MessageDlg(' <<<  A T E N C I O N  >>>   NO SE GENERO CORRELATIVO COMUNIQUESE CON SISTEMAS !!!!!  ', mtError,[mbOk],0);
...
If xFrmPag='31' Then DistExcesos(xAsoId,xAplicar);
Screen.Cursor := crDefault;
...
procedure TfNPago.FormCreate(Sender: TObject);
begin
...
var xCuota,xAmoPag,xIntPag,xGasPag:Currency;
...
begin
   ...
   If xCnd='0' Then
      ...
      DM1.cdsDcuotas.DataRequest('SELECT ASOID, ASOCODMOD, ASOCODPAGO, TIPCREID, TMONID, CREANO, CREMES, CRESALDO,          ' +
                                 ...
                                 'FROM CRE302 Where ASOID ='+QuotedStr(Trim(xAsoId))+' And CREDID='+QuotedStr(Trim(xCreDid))+' And CRECUOTA='+xCreCuota+' And  CREESTID IN (''02'',''11'',''27'',''31'') Order By CREDID,CRECUOTA') ;
      End
   ...
   Else If xCnd='2' Then
      ...
      DM1.cdsDcuotas.DataRequest('SELECT ASOID, ASOCODMOD, ASOCODPAGO, TIPCREID, TMONID, CREANO, CREMES, CRESALDO, FLGINT,         ' +
                                 ...
                                 'CREMTOGADM, ASOCODAUX, NRONABO, CIUDID, FLGPAG,CREDIDNABO, CREMTOEXC, AREAID,CREDOCPAG,CREAPLEXC    ' +
                                 'FROM CRE302 Where ASOID ='+QuotedStr(Trim(xAsoId))+' And CREDID='+QuotedStr(Trim(xCreDid))+' And FLAGVAR=''S'' And  CREESTID IN (''02'',''11'',''27'',''31'') Order By CREDID,CRECUOTA') ;
      End;
   ...
   DM1.cdsDetalle.DataRequest('SELECT CIAID,ASOID,ASOCODMOD,ASOCODAUX,USEID,TIPCREID,CREDID,CRECUOTA,         ' +
                              ...
                              'ASOAPENOM,CIUDID,CREOBS,CREANO,CREMES,AREAID,FORPAGABR,TRANSINTID, FORPAGOABR , ' +
                              'FORPAGID,CREESTID,CREESTADO,FOPERAC,CREESTANT,CREESTANTDES,CREMTOCUO,NROOPE,   ' +
                              'PERCONTA,CREMTOEXC,CREAPLEXC,CNTANOMM,TIPOCONT,FOPERACTMP,NROOPETMP,NRONABO,CNTFLAG,OFDESID,DETRCOBID,RCOBID FROM CRE310 Where ASOID ='+QuotedStr(Trim(xAsoId)));
       ...
       Begin
          If xAplicar>0 Then
              Begin
                ...
                If (xfVenc > xFecCalCulo) And (DM1.cdsDcuotas.fieldbyname('CREFVEN').AsString=DM1.cdsDcuotas.fieldbyname('CREFVENINI').AsString) Then
                   Begin
                   If xCnd='0' Then
                      Begin
                           ...
                           If xCuota > xAplicar Then
                              Begin
                                AplicaCronogramaPago(xAplicar,xExceso,2,'S',xFrmPag); InsertaDetPagos(xAplicar,xExceso,xAmoPag, xIntPag, xGasPag ,1,xEstAnt,xEstDes,xFrmPag,xCnd);
                              End
                           Else
                              Begin
                                AplicaCronogramaPago(xCuota,xExceso,1,'S',xFrmPag); InsertaDetPagos(xCuota,xExceso,xAmoPag, xIntPag, xGasPag ,1,xEstAnt,xEstDes,xFrmPag,xCnd);
                              End;
                      End
                   Else
                      Begin
                        If DM1.cdsDcuotas.fieldbyname('CREMTOCOB').AsCurrency > 0 Then
                           Begin
                              ...
                              If xCuota > xAplicar Then
                                 Begin
                                  AplicaCronogramaPago(xAplicar,xExceso,2,'N',xFrmPag); InsertaDetPagos(xAplicar,xExceso,xAmoPag, xIntPag, xGasPag ,1,xEstAnt,xEstDes,xFrmPag,xCnd);
                                 End
                              Else
                                 Begin
                                  AplicaCronogramaPago(xCuota,xExceso,0,'N',xFrmPag); InsertaDetPagos(xCuota,xExceso,xAmoPag, xIntPag, xGasPag ,1,xEstAnt,xEstDes,xFrmPag,xCnd);
                                 End;
                           End
                        Else
                            Begin
                                ...
                                If xCuota > xAplicar Then
                                   Begin
                                    AplicaCronogramaPago(xAplicar,xExceso,2,'N',xFrmPag); InsertaDetPagos(xAplicar,xExceso,xAmoPag, xIntPag, xGasPag ,0,xEstAnt,xEstDes,xFrmPag,xCnd);
                                   End
                                Else
                                   Begin
                                    AplicaCronogramaPago(xCuota,xExceso,0,'N',xFrmPag); InsertaDetPagos(xCuota,xExceso,xAmoPag, xIntPag, xGasPag ,0,xEstAnt,xEstDes,xFrmPag,xCnd);
                                  End;
                            End;
                      End;
                   End
...
begin
 ...
 If DM1.xSal='RP' Then
    Begin
      If Copy(Trim(fRepCronog1.EdtForPago.Text),1,1)='R' Then xEstId:='12'  Else If Copy(Trim(fRepCronog1.EdtForPago.Text),1,1)='A' Then xEstId:='22'  Else If Copy(Trim(fRepCronog1.EdtForPago.Text),1,1)='C' Then xEstId:='29';
 ...
 If DM1.xSal='RP' Then
    Begin
      DM1.cdsDcuotas.fieldbyname('CREDOCPAG').AsString  := fRepCronog1.xNroAbonoRef;
      DM1.cdsDcuotas.fieldbyname('NROOPE').AsString     := fRepCronog1.xNroAbonoRef;
    End
 Else
    Begin
      If xFrmPag ='03' Then
...
// Inicio: HPP_201102_COB
// Inicio: HPP_201101_COB 
//    DAF-2010-017-COB
{
 DM1.cdsDcuotas.fieldbyname('FOPERAC').AsDateTime := DM1.FechaSys;
}
//   DM1.cdsDcuotas.fieldbyname('FOPERAC').AsDateTime :=DM1.FechasOutPut(DtpFecPag.Date);
// Fin: HPP_201101_COB

// De acuerdo a esta envaluación cambiara la forma como se llena la fecha de Operación
 If DM1.xSal='RP' Then
     DM1.cdsDcuotas.fieldbyname('FOPERAC').AsDateTime := DM1.FechaSys
 Else
     DM1.cdsDcuotas.fieldbyname('FOPERAC').AsDateTime :=DM1.FechasOutPut(DtpFecPag.Date);
...
procedure TfNPago.InsertaDetPagos(xCuotaApl, xExceso, xAmoPag, xIntPag, xGasPag : Currency; xEstado: Integer; xEstAnt, xDesAnt, xFrmPag, xCnd: String);

var xFlgPag:Char;
    xFecCal,xAnoMes,xSQL,xDist:String;
    sUno,sDos,sTres,xSaldo,wSaldo,fSaldo:Currency;
...
//DM1.cdsDetalle.fieldbyname('FOPERAC').AsDateTime := DM1.FechasOutPut(DtpFecPag.Date);
// Fin: HPP_201101_COB
...
If DM1.xGrabaExc='N' Then
  Begin
      ...
      If xEstAnt='27' Then
         Begin
            sUno := 0.00 ; sDos := 0.00 ; sTres := 0.00 ; fSaldo := 0 ; wSaldo:=0;  xDist:='A';

         // Inicio: HPP_201101_COB 
         //    DAF-2010-017-COB
{
            If (StrToInt(xAnoMes) > StrToInt(xFecCal) ) AND (DM1.cdsDcuotas.fieldbyname('CREFVEN').AsString=DM1.cdsDcuotas.fieldbyname('CREFVENINI').AsString) Then xDist:='V';
}
            If (StrToInt(xAnoMes) > StrToInt(xFecCal) ) AND (DM1.cdsDcuotas.fieldbyname('CREFVEN').AsString<>DM1.cdsDcuotas.fieldbyname('CREFVENINI').AsString) Then xDist:='V';
         // Fin: HPP_201101_COB 
            If (StrToInt(xFecCal) >= StrToInt(xAnoMes)) Then xDist:='V';
            If (xCnd='0') Then xDist:='V';
            If xDist='V'  Then
//            If (StrToInt(xFecCal)>=StrToInt(xAnoMes)) Or (xCnd='0') Then
               Begin
                   If xGasPag >= DM1.cdsDcuotas.fieldbyname('CREFLAT').AsCurrency  Then
                      Begin
                    ...
                    Else
                      Begin
                         sUno:=DM1.cdsDcuotas.fieldbyname('CREFLAT').AsCurrency-xGasPag;
                         If sUno >= xCuotaApl Then
                             Begin
                               DM1.cdsDetalle.fieldbyname('CREFLAT').AsCurrency:= xCuotaApl;
                             End
                         Else
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREFLAT').AsCurrency:=sUno;
                                wSaldo:=xCuotaApl-sUno;
                             End;
                      End;
                   If xIntPag >= DM1.cdsDcuotas.fieldbyname('CREINTERES').AsCurrency  Then
                      Begin
                    ...
                    Else
                      Begin
                          sDos:=DM1.cdsDcuotas.fieldbyname('CREINTERES').AsCurrency-xIntPag;
                          If sDos >= wSaldo Then
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREINTERES').AsCurrency:= wSaldo;
                             End
                          Else
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREINTERES').AsCurrency:= sDos;
                                fSaldo:=wSaldo-sDos;
                             End;
                      End;
                   If xAmoPag >= DM1.cdsDcuotas.fieldbyname('CREAMORT').AsCurrency  Then
                      Begin
                         DM1.cdsDetalle.fieldbyname('CREAMORT').AsCurrency:=0.00;
                      End
                    Else
                      Begin
                          sTres:=DM1.cdsDcuotas.fieldbyname('CREAMORT').AsCurrency-xAmoPag;
                          If sTres >= fSaldo Then
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREAMORT').AsCurrency:= fSaldo;
                             End
                          Else
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREAMORT').AsCurrency:= sTres;
                             End;
                      End;
               End
            Else
               Begin
                   ...
                   If xAmoPag >= DM1.cdsDcuotas.fieldbyname('CREAMORT').AsCurrency  Then
                    ...
                    Else
                      Begin
                          sTres:=DM1.cdsDcuotas.fieldbyname('CREAMORT').AsCurrency-xAmoPag;
                          If sTres >= wSaldo Then
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREAMORT').AsCurrency:= wSaldo;
                             End
                          Else
                             Begin
                                DM1.cdsDetalle.fieldbyname('CREAMORT').AsCurrency:= sTres;
      ...
      Else
         Begin
            sUno := 0.00 ; sDos := 0.00 ; xDist:='A';
            // Cambio para casos de reprogramado - Hugo Noriega - 06/02/2008
            If (StrToInt(xAnoMes) > StrToInt(xFecCal) ) AND (DM1.cdsDcuotas.fieldbyname('CREFVEN').AsString<>DM1.cdsDcuotas.fieldbyname('CREFVENINI').AsString) Then xDist:='V';
            //If (StrToInt(xAnoMes) > StrToInt(xFecCal) ) AND (DM1.cdsDcuotas.fieldbyname('CREFVEN').AsString=DM1.cdsDcuotas.fieldbyname('CREFVENINI').AsString) Then xDist:='V';
            If (StrToInt(xFecCal) >= StrToInt(xAnoMes)) Then xDist:='V';
            If (xCnd='0') Then xDist:='V';
...
Else
   Begin
     DM1.cdsDetalle.fieldbyname('CREDOCPAG').AsString :=fRepCronog1.xNroAbonoRef;
     DM1.cdsDetalle.fieldbyname('NROOPE').AsString :=fRepCronog1.xNroAbonoRef;
     DM1.cdsDetalle.fieldbyname('NRONABO').AsString :=fRepCronog1.xNroAbonoRef;
   End;
...
//Importe de Planillas desde Inconsistencias
If (xFrmPag='01') or (xFrmPag='09') or (xFrmPag='14') or (xFrmPag='22') then
 Begin
   if xPlaInc='S' then
      Begin
        xSaldo:=DM1.cdsCEdu.FieldByName('IMPORTE').AsCurrency - DM1.Valores(EdtMonCob.Text);
        DM1.cdsCEdu.Edit;
        DM1.cdsCEdu.FieldByName('IMPORTE').AsCurrency    := xSaldo;
        DM1.cdsDetalle.fieldbyname('DETRCOBID').AsString := DM1.cdsCEdu.FieldByName('DETRCOBID').AsString;
        DM1.cdsDetalle.fieldbyname('RCOBID').AsString    := DM1.cdsCEdu.FieldByName('RCOBID').AsString;
        DM1.cdsDetalle.fieldbyname('FOPERAC').AsString   := DM1.cdsCEdu.FieldByName('FOPERAC').AsString;
        DM1.cdsDetalle.FieldByName('TIPOCONT').AsString  := 'PLA';
        DM1.cdsDetalle.FieldByName('FORPAGOABR').AsString:= TRIM(DBLCTipCob.Text);
        If xSaldo<= 0 Then DM1.cdsCEdu.Delete;
        DM1.cdsCEdu.ApplyUpdates(0);
        DM1.cdsCEdu.Close;
        EdtMonCob.ReadOnly := True;
      End;
...
//Inicio: DPP_201211_COB
    xMoncobDesgrav: Currency;
//Fin: DPP_201211_COB
    xRegistro: TBookmark;
begin
//Inicio: DPP_201211_COB
     DM1.CargaCronograma(xAsoId, xCreDid);
     xCrecapital:=0;xCremtoint:=0;xCremtoflat:=0;xCremtocob:=0;xCremto:=0;xMoncobDesgrav:=0;
     DM1.cdsCuotas.First;
     ...
     Begin
        ...
        xMoncobDesgrav:=  xMoncobDesgrav + DM1.cdsCuotas.fieldbyname('MONCOBDESGRAV').AsCurrency;
...
fCronoPag.dtgPagos.ColumnByName('MONCOBDESGRAV').FooterValue := FloatToStrF(xMoncobDesgrav,ffNumber, 10, 2);
...
Case StrToInt(xFrm) Of
    ...
    18:Begin
       ...
       End;
    31:Begin
         DtpFecPag.Date := StrToDate(FechaAnterior(xAsoid));
         DtpFecPag.Enabled := False;
         EdtDocPag.Enabled := False;
       End
Else
...
begin
 ...
 If xAplicar>0 Then
    Begin
      ...
      InsertaDetPagos(xAplicar,0,0,0,0,1,xEstAnt,xEstDes,xFrmPag,xCnd);
      DM1.cdsDcuotas.ApplyUpdates(0) ; DM1.cdsDetalle.ApplyUpdates(0);
    End;
    DM1.cdsDcuotas.Close;  DM1.cdsDetalle.Close;
// inicio: HPP_200941_COB
// Memorándum 918-2009-DM-COB - SAR-2009-0837 - DAD-RM-2009-0134
// Se retira la resta de lo aplicado al exceso
   //xSQL:='UPDATE CRE302 SET CREMTOEXC=(SELECT SUM(NVL(CREMTOEXC,0)-NVL(CREAPLEXC,0)) FROM CRE310 WHERE ASOID='+QuotedStr(xAsoId)+' AND CREDID='+QuotedStr(xCreDid)+' AND CRECUOTA='+QuotedStr(xCreCuota)+' AND CREESTID NOT IN (''04'',''13'') GROUP BY ASOID,CREDID,CRECUOTA) WHERE ASOID='+QuotedStr(xAsoId)+' AND CREDID='+QuotedStr(xCreDid)+' AND CRECUOTA='+QuotedStr(xCreCuota);
      xSQL:='UPDATE CRE302 '
           +'   SET CREMTOEXC =(SELECT SUM(NVL(CREMTOEXC,0)) '
           +'                     FROM CRE310 '
           +'                    WHERE ASOID    = '+QuotedStr(xAsoId)
           +'                      AND CREDID   = '+QuotedStr(xCreDid)
           +'                      AND CRECUOTA = '+QuotedStr(xCreCuota)
           +'                      AND CREESTID NOT IN (''04'',''13'') '
           +'                    GROUP BY ASOID,CREDID,CRECUOTA) '
           +' WHERE ASOID    = '+QuotedStr(xAsoId)
           +'   AND CREDID   = '+QuotedStr(xCreDid)
           +'   AND CRECUOTA = '+QuotedStr(xCreCuota);
// fin: HPP_200941_COB
   ...
   Begin
     ...
     Else
        Begin
              ...
              EdtMonCob.ReadOnly := False; LblExceso.Enabled := False; EdtMonExc.Enabled := False;
              BitTel.Enabled:=False;
              If (Trim(DBLCTipCob.text)='18') OR (Trim(DBLCTipCob.text)='01') OR (Trim(DBLCTipCob.text)='09') OR
                 (Trim(DBLCTipCob.text)='14') OR (Trim(DBLCTipCob.text)='22') Then
                 Begin
                   EdtMonCob.ReadOnly := True;
                   BitTel.Enabled:=True;
                 End;
...
//Planillas
if (Trim(DBLCTipCob.text)='01') or (Trim(DBLCTipCob.text)='09') or (Trim(DBLCTipCob.text)='14') or (Trim(DBLCTipCob.text)='22') Then
    Begin
     if Length(TRIM(EdtDocPag.Text))=0 then
        begin
          ShowMessage('Debe Ingresar Nro.de Documento');
          EdtDocPag.SetFocus;
          exit;
        end;

      sSql:='SELECT ASOID,ASOAPENOM,IMPORTE,FOPERAC,NROOPE,RCOBID,DETRCOBID,TRANSANO,TRANSMES,CREFPAG,ASOCODMOD,UPROID,UPAGOID,USEID FROM COB340 WHERE'+
            ' ASOID='''+DM1.cdsCuotas.FieldByName('ASOID').AsString+''''+
            ' AND NROOPE='''+TRIM(EdtDocPag.Text)+''''+
            ' AND FOPERAC='''+DateToStr(DtpFecPag.Date)+'''';
       xPlaInc:='S';
    End
else
//Teleahorro
...
function TfNPago.GnrCExe_Ex: String;
var xCorrAct:Integer;
    xCorrExe,xSQL:String;
begin
xSQL:='SELECT CREAREA,TIPO,CANCUO,CORREANO FROM CRE206 WHERE CREAREA=''COB'' AND TIPO=''X'' AND CORREANO='+QuotedStr(Copy(Trim(DateToStr(DM1.FechaSys)),7,4))+' AND CANCUO IS NOT NULL';
DM1.cdsFEfec.Close;
DM1.cdsFEfec.DataRequest(xSQL);
DM1.cdsFEfec.Open;
If DM1.cdsFEfec.RecordCount=1 Then
...
function TfNPago.FechaAnterior(xAsoid: String): String;
var xSQL,xFecha:String;
begin
xSQL:='SELECT MIN(CREFPAG) CREFPAG FROM CRE310 WHERE ASOID='+QuotedStr(xAsoId)+' AND NVL(CREMTOEXC,0)>0 AND CREESTID NOT IN (''13'',''04'') ';
DM1.cdsSitRepr.Close;
DM1.cdsSitRepr.DataRequest(xSQL);
DM1.cdsSitRepr.Open;
If Length(Trim(DM1.cdsSitRepr.FieldByName('CREFPAG').AsString))>0 Then
   Begin
      xFecha:=DM1.cdsSitRepr.FieldByName('CREFPAG').AsString;
   End
Else
   Begin
