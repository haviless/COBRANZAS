...
procedure TFCanDepo.FormKeyPress(Sender: TObject; var Key: Char);
begin
 if key=#13 then
  begin
    key:=#0;
...
procedure TFCanDepo.FormActivate(Sender: TObject);
var
...
begin
  ...
  DM1.cdsBcos.Filter   := '';
  DM1.cdsBcos.Filter := 'BCOTIPCTA = ''B''';
...
procedure TFCanDepo.dbeAsoNomExit(Sender: TObject);
var
 xSQL : String;
begin
  Deshabilita;
...
procedure TFCanDepo.cargabusqueda;
var
 xSql : String;
begin
...
procedure TFCanDepo.LevantaFiltro(xCampo: String);
var
   Mtx : TMant;
begin
   DM1.cdsAsoX.Close;
   ...
   Try
    ...
    Mtx.Filter          := xCampo;
    Mtx.ControlGridDisp := Fprincipal.cgdAutDisk;
    Mtx.DComC           := DM1.DCOM1;
...
procedure TFCanDepo.OnEditAso(Sender: TObject; MantFields: TFields);
var
  xSQL : String;
begin
   xSQL := 'Select * From APO201'
...
procedure TFCanDepo.dbeCodigoModularExit(Sender: TObject);
var
 xSql : String;
begin
  Deshabilita;
...
procedure TFCanDepo.dbgCreditosDblClick(Sender: TObject);
var
 xsql : String;
 xI : Integer;
begin
...
procedure TFCanDepo.dblcFormaPagoExit(Sender: TObject);
var
 sWhere : String;
begin
...
procedure TFCanDepo.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Screen.Cursor := crDefault;
end;

procedure TFCanDepo.Deshabilita;
begin
  bbtnImpri.enabled := False;
...
procedure TFCanDepo.Z2bbtnSumatoriaClick(Sender: TObject);
var
 xI : Integer;
begin
  try
...
procedure TFCanDepo.Z2bbtnBorrarClick(Sender: TObject);
var
 xI : Integer;
begin
 try
    ...
    if not dm1.cdsHijo.active then Exit;
...
procedure TFCanDepo.dblcChqBancoExit(Sender: TObject);
var
 sWhere : String;
begin
  xBancoid := dblcChqBanco.text;
  ...
  dbeChqNum.Text  :=DM1.cdsQry.FieldByName('BANCOPRF').AsString;

  if Length(Trim(edtChqBanco.text)) > 0 then
    DM1.FiltraCDS(DM1.cdsCuentas, 'SELECT * FROM TGE106 WHERE BANCOID='+quotedStr(dblcChqBanco.text)+
                                  ' AND CIAID='+quotedstr(xCiaid));

  //DM1.FiltraCDS(DM1.cdsCBcos, 'SELECT * FROM TGE106 WHERE BANCOID='+
    //            quotedstr(dblcChqBanco.text)+' AND CIAID='+quotedstr(xCiaid));

  xCcBcoid := DM1.cdsCuentas.FieldByName('CCBCOID').AsString;
  xTmonid := DM1.cdsCuentas.FieldByName('TMONID').AsString;
  xVouch  := DM1.cdsCuentas.FieldByName('CCBCOVOUCH').AsString;
...
procedure TFCanDepo.dblctmonedaExit(Sender: TObject);
var
 sWhere : String;
begin
...
procedure TFCanDepo.dbgCronogVenCalcCellColors(Sender: TObject;
...
procedure TFCanDepo.dbgCronogVenFieldChanged(Sender: TObject;
  Field: TField);
var
 xZ,xI : Integer;
 xBookMark : TBookMark;
...
begin
   ...
   if Field.FieldName='FLGINT' then
   begin

      if (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('CREESTID').AsString=xEstid4 then Exit;

      DM1.cdsHijo.FieldByName('SALDO').Readonly := False;

      (Sender as TwwDBGrid).DataSource.DataSet.Edit;
      If (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('FLGINT').AsString='S' then
      begin
         (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('SALDO').AsFloat:=(Sender as TwwDBGrid).DataSource.DataSet.FieldByName('CREAMORT').AsFloat+
      ...
      begin
         if (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('SUMAR').AsString = 'S' then
         begin
            xMto:=xMto+(Sender as TwwDBGrid).DataSource.DataSet.FieldByName('SALDO').AsFloat;
         end;
           ...
           If (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('SUMAR').AsString = 'S' then
                        ...
                        begin
                         ...
                         If xTrue then
                          ...
                          begin
                           ...
                           // VHN
                           (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('FLGINT').AsString:='S';
            ...
            begin
             For xI := 1 to 60 do
              ...
              DM1.cdsHijo.FieldByName('SALDO').Readonly := True;
              // vhn
              (Sender as TwwDBGrid).DataSource.DataSet.FieldByName('FLGINT').AsString:='N';
            end;
...
procedure TFCanDepo.dbeChqNumExit(Sender: TObject);
begin
...
procedure TFCanDepo.bbtnSigueClick(Sender: TObject);
var
    ...
    while not dm1.cdsHijo.eof do
     begin
       If (dm1.cdsHijo.fieldByName('SALDO').AsFloat = 0) or
          ...
          // VHN
          ( ( dm1.cdsHijo.fieldByName('FLGINT').AsString='S' ) AND
        ...
        begin
              ...
              begin
               If (DM1.cdsHijo.FieldByName('CRECAPITAL').Asfloat = 0) AND
                ...
                begin
                  ...
                  if dm1.cdsHijo.fieldByName('FLGINT').AsString='S' then
                  begin
             ...
             if (DM1.cdsHijo.fieldbyname('CREESTID').AsString = xEstid5) or
              ...
              if (DM1.cdsHijo.fieldbyname('CREESTID').AsString = xEstid4) and
               ...
               begin
                  ...
                  // VHN
                  If ( xMtoSal > 0 ) and ( DM1.cdsHijo.FieldByName('FLGINT').AsString='S' ) then
                   begin
                    If xMtoSal > (DM1.cdsHijo.FieldByName('CREINTERES').AsFloat-
                                  DM1.cdsHijo.FieldByName('CREMTOINT').AsFloat) then
                     begin
           ...
           begin
              if DM1.cdsHijo.FieldByName('CREMTOCOB').AsFloat=0 then
              begin
                 xMtoSal:=DM1.cdsHijo.FieldByName('CREAMORT').AsFloat + DM1.cdsHijo.FieldByName('CREFLAT').AsFloat +
                          DM1.cdsHijo.FieldByName('CREINTERES').AsFloat - Dm1.FRound(DM1.cdsHijo.FieldByName('SALDO').AsFloat,15,2);
              end
              else
              begin
...
//                   quotedstr(dbeChqNum.text)+','+wRepFecServi+','+
                  ...
                  ' TO_DATE('+Quotedstr(dbeFOperacion.text)+','+QuotedStr('DD/MM/YYYY')+'),'+
...
{          xFlgInt:='N';
          ...
          // vhn
          {
          xMtoSal := DM1.cdsHijo.FieldByName('CREMTO').AsFloat -  DM1.cdsHijo.FieldByName('SALDO').AsFloat;
          If xMtoSal = 0 then
           xSql := xSql + DM1.cdsHijo.FieldByName('CREMTO').AsString
          else}
           xSql := xSql + FloatToStrF(xMtoSal,ffFixed,15,2);
...
{                         ' ,FOPERAC = TO_DATE('+wRepFecServi+','+QuotedStr('DD/MM/YYY')+')'+
                         ...
                         ' ,FOPERAC = TO_DATE('+Quotedstr(dbeFOperacion.text)+','+QuotedStr('DD/MM/YYYY')+')'+
                         ' ,CREFPAG = TO_DATE('+Quotedstr(dbeFOperacion.text)+','+QuotedStr('DD/MM/YYYY')+')';
...
procedure TFCanDepo.Correlativo;
begin
...
procedure TFCanDepo.CorreCred(pCorre: String);
var
  xCadena,xSql : String;
begin
...
procedure TFCanDepo.BuscaCorre;
var
  xSQL : String;
begin
  //** por defecto
...
procedure TFCanDepo.dbeFOperacionExit(Sender: TObject);
begin
  DecodeDate(dbeFOperacion.Date,xAnos,xMesS,xDiaS);
end;

procedure TFCanDepo.bbtnImpriClick(Sender: TObject);
var
 xSql : String;
begin
...
procedure TFCanDepo.pprReciboPreviewFormCreate(Sender: TObject);
begin
  pprRecibo.PreviewForm.WindowState:=WsMaximized;
  TppViewer(pprRecibo.PreviewForm.Viewer).ZoomSetting:=zsPageWidth;
end;

procedure TFCanDepo.rgFormaClick(Sender: TObject);
var
 xI : Integer;
begin
...
procedure TFCanDepo.CalculoAuto;
var
  xMonto : Double;
  xBook : TBookmark;
  xcompara : boolean;
...
procedure TFCanDepo.CalculoManual;
var
  xMonto : Double;
  xBook : TBookmark;
  xcompara : boolean;
            ...
            If dm1.FRound(xMntDs,15,2) >= dm1.FRound(DM1.cdsHijo.fieldbyname('SALDO').AsFloat,15,2) then
             ...
             begin
               ...
               begin
                  ...
                  DM1.cdsHijo.FieldByName('FLGINT').AsString:='S';
               end
               else
               begin
                  xMntDs := dm1.FRound(DM1.cdsHijo.fieldbyname('SALDO').AsFloat,15,2) - dm1.FRound(xMntDs,15,2);
...
procedure TFCanDepo.bbtnReporteClick(Sender: TObject);
var
 xSql : String;
begin
...
procedure TFCanDepo.bbtnPanelClick(Sender: TObject);
begin
    pnlPagado.visible := False;
    pnlPagado.SendToBack;
end;

procedure TFCanDepo.bbtnImpriPagClick(Sender: TObject);
var
...
procedure TFCanDepo.rgProcesoClick(Sender: TObject);
begin
  Z2bbtnBorrar.OnClick(Z2bbtnBorrar);
  dm1.cdsHijo.First;
  edtMonto.text := '0.00';
...
function TFCanDepo.Ceros(pCantidad: String): String;
var
 xPosi,xI : Integer;
 xSubs : String;
begin
...
procedure TFCanDepo.edtMontoExit(Sender: TObject);
begin
...
procedure TFCanDepo.edtMontoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key In [' ', 'A'..'Z', 'a'..'z',
             '°', '|', '¬', '!', '"', '#', '$', '%', '&', '/', '(', ')',
             '=', '?', '\', '¡', '¿', '@', '¨', '´', '*', '+', '~', '[',
...
procedure TFCanDepo.edtRecibidoKeyPress(Sender: TObject; var Key: Char);
begin
  if Key In [' ', 'A'..'Z', 'a'..'z',
             '°', '|', '¬', '!', '"', '#', '$', '%', '&', '/', '(', ')',
             '=', '?', '\', '¡', '¿', '@', '¨', '´', '*', '+', '~', '[',
...
procedure TFCanDepo.edtRecibidoExit(Sender: TObject);
begin
...
procedure TFCanDepo.bbtnHistorialClick(Sender: TObject);
var
 xSql : String;
begin
  try
...
procedure TFCanDepo.ActivaData(pCredito: STring);
var
 xSql : String;
begin
...
procedure TFCanDepo.ActuCaja301;
Const
...
procedure TFCanDepo.dblcBancoExit(Sender: TObject);
var
  xWhere : String;
begin
...
procedure TFCanDepo.CalculoDevo;
var
  xMonto : Double;
  xBook : TBookmark;
  xcompara : boolean;
...
procedure TFCanDepo.cargaSQLDevo;
var
 xSql : String;
begin
...
procedure TFCanDepo.cargaSQLNormal;
var
 xSql : String;
begin
   xSQL:='SELECT FORPAGOID, FORPAGOABR, CREINTERES, CIAID, TMONID, '+
           'CASE WHEN CREESTID = '+quotedstr(xEstid7)+ ' THEN ''T'' ELSE'+
            ...
            ' CASE WHEN FLGINT=''S'' '+
                 ' THEN FLGINT '+
                 ' ELSE CASE WHEN CREESTID='+quotedstr(xEstid2)+
                           ' THEN ''N'' '+
                           ' ELSE ''S'' '+
                      ' END '+
            ' END FLGINT '+
...
procedure TFCanDepo.rgDevoClick(Sender: TObject);
begin
 lblForma.Caption := 'CANCELACION POR DEVOLUCION';
 dbgCronogVen.DataSource := nil;
 cargaSQLDevo;
...
begin
  ...
  dbgCronogVen.Selected.Add('CREMTO'#9'8'#9'Monto~Cuota');
  dbgCronogVen.Selected.Add('FLGINT'#9'3'#9'INT');
  dbgCronogVen.Selected.Add('SALDO'#9'8'#9'Saldo');
  dbgCronogVen.Selected.Add('CREMTOEXC'#9'7'#9'Monto~Exceso');
  ...
  dbgCronogVen.SetControlType('FLGINT',fctCheckBox, 'S;N');
...
procedure TFCanDepo.rptHistorialPreviewFormCreate(Sender: TObject);
begin
	rptHistorial.PreviewForm.ClientHeight := 500;
	rptHistorial.PreviewForm.ClientWidth  := 650;
